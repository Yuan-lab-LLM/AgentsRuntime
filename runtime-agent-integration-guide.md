...
```json
{
  ...
  "system_info": {
    ...
    "cpu": {
      ...
      "used_cores": 0.25,
      "usage_ready": true,
      "usage_data_source": "cgroup_v2:/sys/fs/cgroup/cpu.stat",
      "load": {
        "1m": 0.64,
        "5m": 0.52,
        "15m": 0.40
...
| `system_info.cpu.available_cores` | number | 核数 | 容器 CPU quota/cpuset 归一化后的可用核数，支持小数 |
| `system_info.cpu.usage_percent` | number | % | 基于 cgroup CPU 累计用量差值计算的容器 CPU 使用率 |
| `system_info.cpu.used_cores` | number | 核数 | 当前采样窗口消耗的 CPU 核数 |
| `system_info.cpu.usage_ready` | boolean | - | 是否已有上一帧样本可计算使用率；首次采样通常为 `false` |
...
CPU 百分比由 agent 按相邻两次 cgroup CPU 累计用量差值计算，并直接上报 `usage_percent`；前端不应再用 `load.1m / cores * 100` 作为 CPU 利用率，只能把 load average 作为参考指标展示。内存百分比由 `(mem_total_bytes - mem_available_bytes) / mem_total_bytes * 100` 计算。磁盘百分比由 `(root_total_bytes - root_free_bytes) / root_total_bytes * 100` 计算。网络速率由相邻两次 `rx_bytes`、`tx_bytes` counter 差值计算，所以 agent 必须上报单调递增的累计 counter，不要把瞬时速率填进这两个字段。

推荐采样来源：

- CPU 使用率：优先读取 cgroup v2 `/sys/fs/cgroup/cpu.stat` 的 `usage_usec`；cgroup v1 读取 `cpuacct.usage`，用相邻两次采样差值除以采样间隔和容器可用 CPU 核数。
- CPU load：`/proc/loadavg`，仅作为 load average 参考，不作为 CPU 利用率。
